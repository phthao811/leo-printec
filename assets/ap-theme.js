(() => {
  ...
  class LocalizationForm extends HTMLElement {
    ...
    openSelector() {
      ...
      this.elements.button.setAttribute('ap-expanded-aria', (this.elements.button.getAttribute('ap-expanded-aria') === 'false').toString());
    }
  ...
  function ApolloEventTrigger(element, name, data = {}) {
    element.dispatchEvent(new CustomEvent(name, {
      bubbles: true,
      detail: data
    }));
  }
  function ApolloTriggerEventNonBubbling(element, name, data = {}) {
    element.dispatchEvent(new CustomEvent(name, {
      bubbles: false,
      detail: data
    }));
  ...
  var CustomHTMLElement = class extends HTMLElement {
    ...
    loadingBarEnable() {
      ApolloEventTrigger(document.documentElement, "theme:loading:start");
    }
    loadingBarDisable() {
      ApolloEventTrigger(document.documentElement, "theme:loading:end");
    }
  ...
  var apollohtmltagSelectors = ["input", "select", "textarea", "a[href]", "button", "[tabindex]", "audio[controls]", "video[controls]", '[contenteditable]:not([contenteditable="false"])', "details>summary:first-of-type", "details"];
  ...
  var indexOfTab = function indexOfTabFunction(node) {
    ...
    let status = (node.nodeName === "AUDIO" || node.nodeName === "VIDEO" || node.nodeName === "DETAILS") ;
  ...
  var fromFieldDisabledStatus = function fromFieldDisabledStatusFunction(node) {
    if (inputStatus(node) || node.tagName === "SELECT" || node.tagName === "TEXTAREA" || node.tagName === "BUTTON") {
  ...
  var nodeFocusMatchingSelectoreStatus = function nodeFocusMatchingSelectoreStatusFunction(options, node) {
    if (node.disabled || hiddenInputStatus(node) || hiddenStatus(node, options.displayCheck) || detailsWithSummaryStatus(node) || fromFieldDisabledStatus(node)) {
      return false;
    }
    return true;
  };
  var nodeSelectorTabbableMatchingStatus = function nodeSelectorTabbableMatchingStatusFunction(options, node) {
    if (!nodeFocusMatchingSelectoreStatus(options, node) || nonRadioTabbableStatus(node) || indexOfTab(node) < 0) {
      return false;
    }
    return true;
  };
  var objTabbable = function objTabbableFunction(el, options) {
    ...
    var apollohtmltags = apolloHtmltag(el, options.includeContainer, nodeSelectorTabbableMatchingStatus.bind(null, options));
    ...
    var objTabbableNodes = orderedTabbables.sort(sortTabOrdered).map(function(a) {
      return a.node;
    }).concat(regularTabbables);
    return objTabbableNodes;
  };
  var focusableCandidateSelector = /* @__PURE__ */ apollohtmltagSelectors.concat("iframe").join(",");
  ...
  var createObjectFocusTrap = function createObjectFocusTrapFunction(elements, userOptions) {
    ...
    var trap;
    var getObjectOption = function getObjectOptionFunction(configOverrideOptions, optionName, configOptionName) {
    ...
    var objectInitialFocusNodeContent = function objectInitialFocusNodeContentFunction() {
      ...
      if (objectNodeForOptionContent("initialFocus") !== null) {
        ...
        var firstTabbableNode = firstObjectGroupTabbable && firstObjectGroupTabbable.firstTabbableNode;
    ...
    var updateObjectTabbableNodes = function updateObjectTabbableNodesFunction() {
      state.objTabbableGroups = state.containers.map(function(container) {
      ...
      if (state.objTabbableGroups.length <= 0 && !objectNodeForOptionContent("fallbackFocus")) {
        throw new Error("Your focus-trap must have at least one container with at least one objTabbable node in it at all times");
      }
    };
    ...
    var objectTabStatus = function objectTabStatusFunction(e) {
      ...
      if (state.objTabbableGroups.length > 0) {
        ...
        if (containerIndex < 0) {
          if (e.shiftKey) {
            destinationNode = state.objTabbableGroups[state.objTabbableGroups.length - 1].lastTabbableNode;
          } else {
          ...
          if (startOfGroupIndex < 0 && state.objTabbableGroups[containerIndex].container === e.target) {
          ...
          if (lastOfGroupIndex < 0 && state.objTabbableGroups[containerIndex].container === e.target) {
    ...
    trap = {
      ...
      deactivate: function deactivate(deactivateOptions) {
        ...
        var returnFocus = getObjectOption(deactivateOptions, "returnFocus", "returnFocusOnDeactivate");
        ...
        if (returnFocus && checkCanReturnFocus) {
          checkCanReturnFocus(contentReturnFocusNode(state.nodeFocusedBeforeActivation)).then(finishDeactivation, finishDeactivation);
          return this;
        }
        finishDeactivation();
        return this;
  ...
  var ApolloOpenableElement = class extends CustomHTMLElement {
    ...
    constructor() {
      super();
      if (Shopify.designMode) {
        this.rootApollo.on("shopify:section:select", (event) => apolloShopifyEventFilter(event, this, () => this.open = true));
        this.rootApollo.on("shopify:section:deselect", (event) => apolloShopifyEventFilter(event, this, () => this.open = false));
      }
    ...
    get focusTrap() {
      return this._focusTrap = this._focusTrap || createObjectFocusTrap(this, {
        fallbackFocus: this,
        initialFocus: this.getAttribute("initial-focus-selector"),
        clickOutsideDeactivates: (event) => !(event.target.hasAttribute("ap-controlsaria") && event.target.getAttribute("ap-controlsaria") === this.id),
        allowOutsideClick: (event) => event.target.hasAttribute("ap-controlsaria") && event.target.getAttribute("ap-controlsaria") === this.id,
      ...
      switch (name) {
        case "open":
          if (oldval === null && newval === "") {
            ...
            ApolloEventTrigger(this, "ap-elementopen:open");
          } else if (newval === null) {
            if (this.shouldTrapFocus) {
              this.focusTrap.deactivate();
            }
            ApolloEventTrigger(this, "ap-elementopen:close");
          }
      }
    }
    async _load() {
      ...
      ApolloTriggerEventNonBubbling(this, "ap-elementopen:load:start");
      ...
      ApolloTriggerEventNonBubbling(this, "ap-elementopen:load:end");
    }
  };
    ...
    attributeChangedCallback(name) {
      ...
      switch (name) {
        case "open":
          ...
          ApolloEventTrigger(this, aptriger);
      }
    }
  };
  ...
  var ApPagePagination = class extends CustomHTMLElement {
    ...
    _onLinkClicked(event, target) {
      ...
      ApolloEventTrigger(this, "pagination:page-changed", { url: url.toString() });
    }
  };
  ...
  var ApToggleButton = class extends HTMLButtonElement {
    ...
    connectedCallback() {
      ...
      this.rootApollo.on("ap-elementopen:load:start", `#${this.getAttribute("ap-controlsaria")}`, () => {
      ...
      this.rootApollo.on("ap-elementopen:load:end", `#${this.getAttribute("ap-controlsaria")}`, () => {
        if (this.classList.contains("button")) {
  ...
  var apToggleLink = class extends HTMLAnchorElement {
    ...
    connectedCallback() {
      this.rootApollo.on("ap-elementopen:close", `#${this.getAttribute("ap-controlsaria")}`, (event) => {
        if (this.objectElementControlled === event.target) {
          this.expandedStatus = false;
        }
      }, true);
      this.rootApollo.on("ap-elementopen:open", `#${this.getAttribute("ap-controlsaria")}`, (event) => {
        if (this.objectElementControlled === event.target) {
          this.expandedStatus = true;
        }
      }, true);
  ...
  var apPageDots = class extends CustomHTMLElement {
      ...
      if (this.hasAttribute("animation-timer")) {
        this.apollo.on("animationend", (event) => {
          if (event.elapsedTime > 0) {
            this._apEvtDispatch((this.selectedIndex + 1 + this.buttons.length) % this.buttons.length);
          }
        });
      }
    }
    ...
    set selectedIndex(selectedIndex) {
      this.buttons.forEach((button, index) => button.setAttribute("ap-currentaria", selectedIndex === index ? "true" : "false"));
      if (this.hasAttribute("ap-selectedalign")) {
        ...
        if (scrollableElement) {
          let left = scrollableElement.scrollLeft + (apBoundingRect.left - windowHalfWidth) + apBoundingRect.width / 2;
          let behavior ="smooth"
    ...
    _apEvtDispatch(index) {
      if (index !== this.selectedIndex) {
        this.dispatchEvent(new CustomEvent("ap-pagedots:changed", {
          bubbles: true,
          detail: {
            index
          }
  ...
  var apButtonsNextPrevious = class extends HTMLElement {
    connectedCallback() {
      ...
      this.prevButton.addEventListener("click", () => this.prevButton.dispatchEvent(new CustomEvent("ap-nextprev:prev", { bubbles: true })));
      this.nextButton.addEventListener("click", () => this.nextButton.dispatchEvent(new CustomEvent("ap-nextprev:next", { bubbles: true })));
    }
  ...
  var apPrevButton = class extends HTMLButtonElement {
    connectedCallback() {
      this.addEventListener("click", () => this.dispatchEvent(new CustomEvent("ap-nextprev:prev", { bubbles: true })));
    }
  };
  var apNextButton = class extends HTMLButtonElement {
    connectedCallback() {
      this.addEventListener("click", () => this.dispatchEvent(new CustomEvent("ap-nextprev:next", { bubbles: true })));
    }
  };
  ...
  function apStickyHeaderOffsetContent() {
    ...
    return parseInt(documentStyles.getPropertyValue("--header-height") || 0) * parseInt(documentStyles.getPropertyValue("--enable-sticky-header") || 0) + parseInt(documentStyles.getPropertyValue("--ap-announcementbar-height") || 0) * parseInt(documentStyles.getPropertyValue("--enable-sticky-ap-announcementbar") || 0);
  }
  ...
  function apStream(callback, delay3 = 15) {
    let apStreamTimeout = null, storedEvent = null;
  ...
  var ApUpdateProcess = class {
    constructor(targetElement) {
      this.scheduleUpdate = apStream(() => this.updateProcess(targetElement, getComputedStyle(targetElement)));
    ...
    updateProcess(targetElement, style) {
      if (this.element) {
        const maxSize = style.getPropertyValue("--ap-shadowscroll-size") ? parseInt(style.getPropertyValue("--ap-shadowscroll-size")) : 0;
        const scroll = {
  ...
  window.customElements.define("ap-sharetogglebutton", ApShareApToggleButton, { extends: "button" });
  ...
  var APCarousel = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      if (this.items.length > 1) {
        ...
        this.addEventListener("ap-pagedots:changed", (event) => this.select(event.detail.index, true));
        if (Shopify.designMode) {
      ...
      this.intersectionObserver = new IntersectionObserver(this._onVisibilityChanged.bind(this), { root: scrollerElement, rootMargin: rootMargin, threshold: 0.8 });
    ...
    select(index, useEffect = true) {
      ...
      selectedElement.parentElement.scrollTo({ left: direction * (selectedElement.clientWidth * clampIndex), behavior: useEffect ? "smooth" : "auto" });
    }
  ...
  var ApScrollableContent = class extends CustomHTMLElement {
    ...
    _apDraggSetup() {
      this.insertAdjacentHTML("afterend", '<ap-cursorofdrag hidden class="custom-ap-cursorofdrag"></ap-cursorofdrag>');
    ...
    _apProgressCalculator() {
      ...
      ApolloEventTrigger(this, "ap-scrollablecontent:progress", { progress });
    }
    ...
    split(force = false) {
      ...
      return new Promise((resolve) => {
        requestAnimationFrame(() => {
          ...
          this.innerHTML = Object.values(bounds).map((item) => `<span ${this.hasAttribute("reveal") && !force ? "reveal" : ""} ${this.hasAttribute("ap-revealvisibility") && !force ? "ap-revealvisibility" : ""} style="display: block">${item.trim()}</span>`).join("");
  ...
  var ApNavTabs = class extends HTMLElement {
    ...
    selectButton(button, animate = true) {
      ...
      const from = document.getElementById(this.selectedButton.getAttribute("ap-controlsaria")), to = document.getElementById(button.getAttribute("ap-controlsaria"));
      if (animate) {
      ...
      ApolloEventTrigger(this, "ap-navtabs:changed", { button });
      this._apEvtNavigationPositionChange();
    }
    addButton(button) {
    ...
    _apEvtNavigationPositionChange(useEffect = true) {
      ...
      let left = this.selectedButton.offsetLeft - windowHalfWidth + this.selectedButton.clientWidth / 2
  ...
  var ApLocalizationSelector = class extends HTMLSelectElement {
    connectedCallback() {
      ...
      if (this.localizationProvinceElement.hasAttribute("data-default")) {
        for (let i = 0; i !== this.localizationProvinceElement.options.length; ++i) {
          if (this.localizationProvinceElement.options[i].text === this.localizationProvinceElement.getAttribute("data-default")) {
    ...
    _updateLocalizationProvinceVisibility() {
      ...
      let localizationProvinceSelectElement = this.localizationProvinceElement.querySelector("select");
      if (localizationProvinceSelectElement = this.localizationProvinceElement.tagName === "SELECT") {
  ...
  window.customElements.define("ap-localizationselector", ApLocalizationSelector, { extends: "select" });
  ...
  var ApPriceRange = class extends HTMLElement {
    connectedCallback() {
      ...
      this.textInputMaxBound.addEventListener("change", (event) => {
        event.target.value = Math.min(Math.max(parseInt(event.target.value), parseInt(this.textInputMinBound.value || event.target.min) + 1), event.target.max);
      ...
      this.textInputMinBound.addEventListener("change", (event) => {
        event.target.value = Math.max(Math.min(parseInt(event.target.value), parseInt(this.textInputMaxBound.value || event.target.max) - 1), event.target.min);
      ...
      this.rangeMaxBound.addEventListener("input", (event) => {
        ApolloEventTrigger(this, "facet:abort-loading");
        event.target.value = Math.max(parseInt(event.target.value), parseInt(this.textInputMinBound.value || event.target.min) + 1);
      ...
      this.rangeMinBound.addEventListener("input", (event) => {
        ApolloEventTrigger(this, "facet:abort-loading");
        event.target.value = Math.min(parseInt(event.target.value), parseInt(this.textInputMaxBound.value || event.target.max) - 1);
        let val = parseInt(event.target.value) / parseInt(event.target.max) * 100;
      ...
      async _apFlickityCreate() {
        this.flickity = new Promise(async (resolve) => {
          ...
          const flickityInstance = new window.ThemeFlickity(this, { ...this.flickityConfig, ...{
            ...
            on: {
              ready: (event) => ApolloEventTrigger(this, "flickity:ready", event),
              change: (event) => ApolloEventTrigger(this, "flickity:slide-changed", event),
              settle: (event) => ApolloEventTrigger(this, "flickity:slide-settled", event)
            }
          } });
  ...
  var ApFlickityControls = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      this.apollo.on("click", '[data-action="select"]', (event, target) => this.flickityCarousel.select(`#${target.getAttribute("ap-controlsaria")}`));
    }
    ...
    async _onSlideChanged(animate = true) {
      ...
      let flickityactiveItems = Array.from(this.querySelectorAll(`[ap-controlsaria="${flickityInstance.selectedElement.id}"]`));
  ...
  var ApComboBox = class extends ApolloOpenableElement {
    connectedCallback() {
      ...
      if (this.hasAttribute("fit-toggle")) {
        const maxWidth = Math.max(...this.options.map((item) => item.clientWidth)), control = document.querySelector(`[ap-controlsaria="${this.id}"]`);
        if (control) {
  ...
  var ApQuantitySelector = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.apollo.on("click", "button:first-child", () => this.inputElement.quantity = this.inputElement.quantity - 1);
      this.apollo.on("click", "button:last-child", () => this.inputElement.quantity = this.inputElement.quantity + 1);
    }
  };
  ...
  var CustomAnnouncementBarItem = class extends CustomHTMLElement {
    ...
    get focusTrap() {
      return this._trapFocus = this._trapFocus || createObjectFocusTrap(this.contentElement.querySelector(".ap-announcementbar__content-inner"), {
        fallbackFocus: this,
  ...
  var SearchPage = class extends HTMLElement {
    ...
    //     promisesList.push(fetch(`${window.themeVariables.routes.searchUrl}?section_id=${this.getAttribute("section-id")}&q=${this.terms}&type=${item}&options[prefix]=last&options[unavailable_products]=${window.themeVariables.settings.searchProductsUnavailable}`));
    //   });
    //   const responses = await Promise.all(promisesList);
    //   await Promise.all(responses.map(async (response) => {
    //     const div = document.createElement("div");
    //     div.innerHTML = await response.text();
    //     const categoryResultDiv = div.querySelector(".main-search__category-result"), tabNavItem = div.querySelector("#search-ap-navtabs .ap-navtabs__item");
  ...
  ApProductRecommendations = class extends HTMLElement {
    async connectedCallback() {
      ...
      const response = await fetch(`${window.themeVariables.routes.productUrlRecommendations}?product_id=${this.productId}&limit=${this.recommendationsCount}&section_id=${this.sectionId}`);
      const div = document.createElement("div");
      div.innerHTML = await response.text();
  ...
  var ApRecentlyProductsViewed = class extends HTMLElement {
    async connectedCallback() {
      ...
      const response = await fetch(`${window.themeVariables.routes.apDrawerSearchUrl}?q=${this.searchQueryString}&resources[limit]=10&resources[type]=product&section_id=${this.sectionId}`);
      const div = document.createElement("div");
      div.innerHTML = await response.text();
  ...
  var ApLoadMoreProducts = class extends HTMLElement {
    ...
    _nextpage(){
      ...
      let limit = parseInt(document.getElementById('ap_page_limit').value);
      let nextpage = page + 1;
      let url = document.getElementById('ap_page_link').value + '/products.json?page=' + nextpage + '&limit=' + limit;
      fetch(`${url}`)
      .then(x => x.text())
      .then(y => this._loadcontentnextpage(y));
    }

    _loadcontentnextpage(val){
      let products = JSON.parse(val).products;
      if (products.length) {
        document.getElementById('loading-more-product-icon').style.display = 'block';
        for (let i=0; i<products.length; i++) {
          let ids = 'id:' + products[i].id;
          if (i < products.length - 1) {
            fetch(`${window.themeVariables.routes.apDrawerSearchUrl}?q=${ids}&resources[limit]=1&resources[type]=product&section_id=${this.getAttribute("section-id")}`)
            .then(x => x.text())
            .then(y => this._updatehtml(y));
          } else {
            fetch(`${window.themeVariables.routes.apDrawerSearchUrl}?q=${ids}&resources[limit]=1&resources[type]=product&section_id=${this.getAttribute("section-id")}`)
            .then(x => x.text())
            .then(y => this._updatehtmlend(y));
          }
  ...
  var ApEffectOfCustomKeyframe = class {
    ...
    get finished() {
      ...
      return this._customanimation.finished ? this._customanimation.finished : new Promise((resolve) => this._customanimation.onfinish = resolve);
    }
    play() {
      if (this._customanimation) {
  ...
  var ApSlideshowItem = class extends HTMLElement {
    ...
    async transitionToLeave(transitionType, useEffect = true) {
      ...
      this._pendingAnimations = [];
      let animation = null, textElements = await resolveAsyncIterator(this.querySelectorAll("ap-splitlines, .button-group, .button-wrapper , .slideshow__image-child"));
      let elementImages = Array.from(this.querySelectorAll(".slideshow__image-wrapper"));
      switch (transitionType) {
        case "sweep":
          animation = new ApAnimationCustom(new ApEffectSequence([
            new ApEffectOfCustomKeyframe(this, { visibility: ["visible", "hidden"] }, { duration: 500 }),
            new ApEffectParallel(textElements.map((item) => {
              return new ApEffectOfCustomKeyframe(item, { opacity: [1, 0], visibility: ["visible", "hidden"] });
            }))
          ]));
          break;
        case "fade":
          animation = new ApAnimationCustom(new ApEffectOfCustomKeyframe(this, { opacity: [1, 0], visibility: ["visible", "hidden"] }, { duration: 250, easing: "ease-in-out" }));
          break;
        case "reveal":
          animation = new ApAnimationCustom(new ApEffectSequence([
            new ApEffectParallel(textElements.reverse().map((item) => {
              return new ApEffectOfCustomKeyframe(item, { opacity: [1, 0], visibility: ["visible", "hidden"] }, { duration: 250, easing: "ease-in-out" });
            })),
            new ApEffectParallel(elementImages.map((item) => {
              if (!item.classList.contains("slideshow__image-wrapper--secondary")) {
                return new ApEffectOfCustomKeyframe(item, { visibility: ["visible", "hidden"], clipPath: ["inset(0 0 0 0)", "inset(0 0 100% 0)"] }, { duration: 450, easing: "cubic-bezier(0.99, 0.01, 0.50, 0.94)" });
              } else {
                return new ApEffectOfCustomKeyframe(item, { visibility: ["visible", "hidden"], clipPath: ["inset(0 0 0 0)", "inset(100% 0 0 0)"] }, { duration: 450, easing: "cubic-bezier(0.99, 0.01, 0.50, 0.94)" });
              }
            }))
          ]));
          break;
    ...
    async transitionToEnter(transitionType, useEffect = true, reverseDirection = false) {
      ...
      let animation = null, textElements = await resolveAsyncIterator(this.querySelectorAll("ap-splitlines, .button-group, .button-wrapper")), elementImages = Array.from(this.querySelectorAll(".slideshow__image-wrapper"));
      switch (transitionType) {
        case "sweep":
          animation = new ApAnimationCustom(new ApEffectSequence([
            new ApEffectOfCustomKeyframe(this, { visibility: ["hidden", "visible"], clipPath: reverseDirection ? ["inset(0 100% 0 0)", "inset(0 0 0 0)"] : ["inset(0 0 0 100%)", "inset(0 0 0 0)"] }, { duration: 500, easing: "cubic-bezier(1, 0, 0, 1)" }),
            new ApEffectParallel(textElements.map((item, index) => {
              return new ApEffectOfCustomKeyframe(item, { opacity: [0, 1], visibility: ["hidden", "visible"], clipPath: ["inset(0 0 100% 0)", "inset(0 0 0 0)"], transform: ["translateY(100%)", "translateY(0)"] }, { duration: 450, delay: 100 * index, easing: "cubic-bezier(0.5, 0.06, 0.01, 0.99)" });
            }))
          ]));
          break;
        case "fade":
          animation = new ApAnimationCustom(new ApEffectOfCustomKeyframe(this, { opacity: [0, 1], visibility: ["hidden", "visible"] }, { duration: 250, easing: "ease-in-out" }));
          break;
        case "reveal":
          animation = new ApAnimationCustom(new ApEffectSequence([
            new ApEffectParallel(elementImages.map((item) => {
              if (!item.classList.contains("slideshow__image-wrapper--secondary")) {
                return new ApEffectOfCustomKeyframe(item, { visibility: ["hidden", "visible"], clipPath: ["inset(0 0 100% 0)", "inset(0 0 0 0)"] }, { duration: 450, delay: 100, easing: "cubic-bezier(0.5, 0.06, 0.01, 0.99)" });
              } else {
                return new ApEffectOfCustomKeyframe(item, { visibility: ["hidden", "visible"], clipPath: ["inset(100% 0 0 0)", "inset(0 0 0 0)"] }, { duration: 450, delay: 100, easing: "cubic-bezier(0.5, 0.06, 0.01, 0.99)" });
              }
            })),
            new ApEffectParallel(textElements.map((item, index) => {
              return new ApEffectOfCustomKeyframe(item, { opacity: [0, 1], visibility: ["hidden", "visible"], clipPath: ["inset(0 0 100% 0)", "inset(0 0 0 0)"], transform: ["translateY(100%)", "translateY(0)"] }, { duration: 450, delay: 100 * index, easing: "cubic-bezier(0.5, 0.06, 0.01, 0.99)" });
  ...
  var ApSlideshow = class extends CustomHTMLElement {
    ...
    async _setupTextOverlayImageVisibility() {
      ...
      await this.listItems[this.selectedIndex].transitionToEnter(this.transitionType).catch((error) => {
      });
      this.startPlayer();
    }
    previous() {
      this.select((this.selectedIndex - 1 + this.listItems.length) % this.listItems.length, true, true);
    }
    next() {
      this.select((this.selectedIndex + 1 + this.listItems.length) % this.listItems.length, true, false);
    }
    async select(index, shouldTransition = true, reverseDirection = false) {
      ...
      const itemPrevious = this.listItems[this.selectedIndex], newItem = this.listItems[index];
  ...
  var ApApImageWithTextItem = class extends HTMLElement {
    ...
    async transitionToEnter(useEffect = true) {
      this.removeAttribute("hidden");
      const textWrapper = this.querySelector(".ap-imagewithtext__text-wrapper"), headings = await resolveAsyncIterator(this.querySelectorAll(".ap-imagewithtext__content ap-splitlines"));
      const animation = new ApAnimationCustom(new ApEffectSequence([
        new ApEffectParallel(headings.map((item, index) => {
    ...
    async transitionToLeave(useEffect = true) {
      const elements = await resolveAsyncIterator(this.querySelectorAll(".ap-imagewithtext__text-wrapper, .ap-imagewithtext__content ap-splitlines"));
      const animation = new ApAnimationCustom(new ApEffectParallel(elements.map((item) => {
  ...
  var ApImageWithText = class extends CustomHTMLElement {
    ...
    async transitionImage(index, useEffect = true) {
      ...
      const animation = new ApAnimationCustom(new ApEffectOfCustomKeyframe(nextImage, {
        visibility: ["hidden", "visible"],
  ...
  var ApTestimonialItem = class extends CustomHTMLElement {
    ...
    async transitionToLeave(useEffect = true) {
      const textLines = await resolveAsyncIterator(this.querySelectorAll("ap-splitlines, .testimonial__author, .testimonial__icon")), animation = new ApAnimationCustom(new ApEffectParallel(textLines.reverse().map((item, index) => {
        return new ApEffectOfCustomKeyframe(item, {
  ...
  var ApTheLookbookShopItem = class extends HTMLElement {
    ...
    async transitionToLeave(useEffect = true) {
      this.setAttribute("hidden", "");
      const animation = new ApAnimationCustom(new ApEffectOfCustomKeyframe(this, { visibility: ["visible", "hidden"] }, { duration: 500 }));
      useEffect ? animation.play() : animation.finish();
      return animation.finished;
    }
    async transitionToEnter(useEffect = true) {
      ...
      const animation = new ApAnimationCustom(new ApEffectSequence([
        new ApEffectParallel(Array.from(this.querySelectorAll(".ap-lookbookshop__image")).map((item) => {
          return new ApEffectOfCustomKeyframe(item, { opacity: [1, 1] }, { duration: 0 });
        })),
        new ApEffectOfCustomKeyframe(this, { visibility: ["hidden", "visible"], zIndex: [0, 1], clipPath: ["inset(0 0 0 100%)", "inset(0 0 0 0)"] }, { duration: 500, easing: "cubic-bezier(1, 0, 0, 1)" }),
        new ApEffectParallel(dots.map((item, index) => {
          return new ApEffectOfCustomKeyframe(item, { opacity: [0, 1], transform: ["scale(0)", "scale(1)"] }, { duration: 120, delay: 75 * index, easing: "ease-in-out" });
        }))
      ]));
      useEffect ? animation.play() : animation.finish();
      await animation.finished;
      if (window.matchMedia(window.themeVariables.breakpoints.tabletAndUp).matches) {
        const firstPopover = this.querySelector(".ap-lookbookshop__product-wrapper .ap-lookbookshop__dot");
  ...
  var ApTheLookbookShopNav = class extends CustomHTMLElement {
    ...
    transitionToIndex(index, useEffect = true) {
      const indexElements = Array.from(this.querySelectorAll(".ap-lookbookshop__counter-page-transition")), currentObjectElement = indexElements.find((item) => !item.hasAttribute("hidden")), nextElement = indexElements[index];
      currentObjectElement.animate({ transform: ["translateY(0)", "translateY(-100%)"] }, { duration: useEffect ? 1e3 : 0, easing: "cubic-bezier(1, 0, 0, 1)" }).onfinish = () => currentObjectElement.setAttribute("hidden", "");
      nextElement.removeAttribute("hidden");
      nextElement.animate({ transform: ["translateY(100%)", "translateY(0)"] }, { duration: useEffect ? 1e3 : 0, easing: "cubic-bezier(1, 0, 0, 1)" });
    }
  };
  ...
  var TheLookbookShop = class extends CustomHTMLElement {
    ...
    async _setupTextOverlayImageVisibility() {
      await this.untilObjectVisible();
      const images = Array.from(this.lookItems[this.selectedIndex].querySelectorAll(".ap-lookbookshop__image"));
  ...
  var ApCollectionList = class extends CustomHTMLElement {
    ...
    async _setupTextOverlayImageVisibility() {
      ...
      const animation = new ApAnimationCustom(new ApEffectParallel(this.items.map((item, index) => {
        return new ApEffectSequence([
          new ApEffectOfCustomKeyframe(item.querySelector(".list-collections__item-image"), {
            opacity: [0, 1],
          ...
          new ApEffectParallel(Array.from(item.querySelectorAll(".list-collections__item-info [reveal]")).map((textItem, subIndex) => {
            return new ApEffectOfCustomKeyframe(textItem, {
              opacity: [0, 1],
  ...
  var ApProductList = class extends CustomHTMLElement {
    ...
    get apparitionAnimation() {
      return this._customanimation = this._customanimation || new ApAnimationCustom(new ApEffectParallel(this.productItems.map((item, index) => {
        return new ApEffectOfCustomKeyframe(item, {
          opacity: [0, 1],
    ...
    previous(event) {
      const directionFlip = window.themeVariables.settings.direction === "ltr" ? 1 : -1, columnGap = parseInt(getComputedStyle(this).getPropertyValue("--ap-productlist-column-gap"));
      ...
      switch (name) {
        case "hidden":
          if (!this.hidden) {
            ...
            (_a = this.querySelector(".ap-nextap-buttonprev--prev")) == null ? void 0 : _a.setAttribute("disabled", "");
            (_b = this.querySelector(".ap-nextap-buttonprev--next")) == null ? void 0 : _b.removeAttribute("disabled");
            this._staggerProductsApparition();
          } else {
    ...
    async _staggerProductsApparition() {
      ...
      await this.untilObjectVisible({ threshold: this.clientHeight > 0 ? 50 / this.clientHeight : 0 });
  ...
  var ApBlogPostNavigation = class extends HTMLElement {
    ...
    _updateObjectProgressBar() {
      const apOffsetStickyHeader = apStickyHeaderOffsetContent(), marginCompensation = window.matchMedia(window.themeVariables.breakpoints.pocket).matches ? 40 : 80, articleNavBoundingBox = this.getBoundingClientRect(), articleMainPartBoundingBox = this.parentElement.getBoundingClientRect(), difference = articleMainPartBoundingBox.bottom - (articleNavBoundingBox.bottom - marginCompensation), progress = Math.max(-1 * (difference / (articleMainPartBoundingBox.height + marginCompensation) - 1), 0);
      this.classList.toggle("is-visible", articleMainPartBoundingBox.top < apOffsetStickyHeader && articleMainPartBoundingBox.bottom > apOffsetStickyHeader + this.clientHeight - marginCompensation);
  ...
  var ApMultiColumn = class extends CustomHTMLElement {
    ...
    async _setupTextOverlayImageVisibility() {
      ...
      const prefersReducedMotion = MediaFeatures.prefersReducedMotion();
      const animation = new ApAnimationCustom(new ApEffectParallel(Array.from(this.querySelectorAll(".ap-multicolumn__item")).map((item, index) => {
        return new ApEffectOfCustomKeyframe(item, {
          opacity: [0, 1],
          transform: [`translateY(${MediaFeatures.prefersReducedMotion() ? 0 : window.innerWidth < 1e3 ? 35 : 60}px)`, "translateY(0)"]
        }, {
    ...
    previous(event) {
      const directionFlip = window.themeVariables.settings.direction === "ltr" ? 1 : -1, columnGap = parseInt(getComputedStyle(this).getPropertyValue("--ap-multicolumn-column-gap"));
      event.target.nextElementSibling.removeAttribute("disabled");
  ...
  var ApGalleryList = class extends HTMLElement {
    connectedCallback() {
      ...
      if (this.listItems.length > 1) {
        this.addEventListener("ap-scrollablecontent:progress", this._updateObjectProgressBar.bind(this));
        this.addEventListener("ap-nextprev:prev", this.previous.bind(this));
        this.addEventListener("ap-nextprev:next", this.next.bind(this));
        if (Shopify.designMode) {
          this.addEventListener("shopify:block:select", (event) => this.select(event.target.index, !event.detail.load));
  ...
  var ApTextOverlayImage = class extends CustomHTMLElement {
    ...
    async _setupTextOverlayImageVisibility() {
      await this.untilObjectVisible();
      const image = this.querySelector(".image-overlay__image"), headings = await resolveAsyncIterator(this.querySelectorAll("ap-splitlines")), prefersReducedMotion = MediaFeatures.prefersReducedMotion();
      await imageLoaded(image);
      const innerEffect = [
        new ApEffectOfCustomKeyframe(image, { opacity: [0, 1], transform: [`scale(${prefersReducedMotion ? 1 : 1.1})`, "scale(1)"] }, { duration: 500, easing: "cubic-bezier(0.65, 0, 0.35, 1)" }),
        new ApEffectParallel(headings.map((item, index) => {
          return new ApEffectOfCustomKeyframe(item, {
            opacity: [0, 0.2, 1],
            transform: [`translateY(${prefersReducedMotion ? 0 : "100%"})`, "translateY(0)"],
        ...
        new ApEffectOfCustomKeyframe(this.querySelector(".image-overlay__text-container"), { opacity: [0, 1] }, { duration: 300 })
      ];
      const animation = prefersReducedMotion ? new ApAnimationCustom(new ApEffectParallel(innerEffect)) : new ApAnimationCustom(new ApEffectSequence(innerEffect));
      this._sectionReloadedStatus ? animation.finish() : animation.play();
    }
    _onScrollTextOverlayImage() {
      ...
      requestAnimationFrame(() => {
        const apBoundingRect = this.getBoundingClientRect(), speedFactor = 3, contentElement = this.querySelector(".image-overlay__content-wrapper"), elementImage = this.querySelector(".image-overlay__image"), apBoundingRectBottom = apBoundingRect.bottom, apBoundingRectHeight = apBoundingRect.height, apOffsetStickyHeader = apStickyHeaderOffsetContent();
        if (contentElement) {
          contentElement.style.opacity = Math.max(1 - speedFactor * (1 - Math.min(apBoundingRectBottom / apBoundingRectHeight, 1)), 0).toString();
        }
        if (elementImage) {
          elementImage.style.transform = `translateY(${100 - Math.max(1 - (1 - Math.min(apBoundingRectBottom / (apBoundingRectHeight + apOffsetStickyHeader), 1)), 0) * 100}px)`;
        }
        this._hasPendingRaF = false;
      });
    }
  ...
  var ApImageTextBlock = class extends CustomHTMLElement {
    ...
    async _setupTextOverlayImageVisibility() {
      await this.untilObjectVisible();
      const images = Array.from(this.querySelectorAll(".ap-imagetextblock__image[reveal]")), headings = await resolveAsyncIterator(this.querySelectorAll("ap-splitlines")), prefersReducedMotion = MediaFeatures.prefersReducedMotion();
      ...
      const innerEffect = [
        new ApEffectParallel(images.map((item) => {
          return new ApEffectOfCustomKeyframe(item, { opacity: [0, 1], transform: [`scale(${prefersReducedMotion ? 1 : 1.1})`, "scale(1)"] }, { duration: 500, easing: "cubic-bezier(0.65, 0, 0.35, 1)" });
        })),
        new ApEffectOfCustomKeyframe(this.querySelector(".ap-imagetextblock__content"), { opacity: [0, 1], transform: [`translateY(${prefersReducedMotion ? 0 : "60px"})`, "translateY(0)"] }, { duration: 150, easing: "ease-in-out" }),
        new ApEffectParallel(headings.map((item, index) => {
          return new ApEffectOfCustomKeyframe(item, {
            opacity: [0, 0.2, 1],
            transform: [`translateY(${prefersReducedMotion ? 0 : "100%"})`, "translateY(0)"],
        ...
        new ApEffectOfCustomKeyframe(this.querySelector(".ap-imagetextblock__text-container"), { opacity: [0, 1] }, { duration: 300 })
      ];
      const animation = prefersReducedMotion ? new ApAnimationCustom(new ApEffectParallel(innerEffect)) : new ApAnimationCustom(new ApEffectSequence(innerEffect));
      this._sectionReloadedStatus ? animation.finish() : animation.play();
    }
  ...
  var ApListArticle = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      if (this.staggerApparition) {
        ...
        const animation = new ApAnimationCustom(new ApEffectParallel(this.articleItems.map((item, index) => {
          let transform = 0;
          if (MediaFeatures.prefersReducedMotion()) {
            transform = 0;
          } else {
  ...
  var ApPredictiveSearchInput = class extends HTMLInputElement {
    connectedCallback() {
      this.addEventListener("click", () => document.getElementById(this.getAttribute("ap-controlsaria")).open = true);
    }
  };
  window.customElements.define("ap-predictivesearchinput", ApPredictiveSearchInput, { extends: "input" });
  ...
  var ApSearchDrawer = class extends ApDrawerContent {
    connectedCallback() {
      ...
      this.apollo.on("click", '[data-action="reset-search"]', this._apStartWithNewSearch.bind(this));
    }
    ...
    async _apLiquidWithSearch(term) {
      ...
      supportedTypes.forEach((searchType) => {
        promises.push(fetch(`${window.themeVariables.routes.searchUrl}?section_id=predictive-search-compatibility&q=${term}&type=${searchType}&options[unavailable_products]=${window.themeVariables.settings.searchProductsUnavailable}&options[prefix]=last`, {
          signal: this.abortController.signal
        }));
      });
  ...
  var ApTimeline = class extends HTMLElement {
    connectedCallback() {
      ...
      if (this.listItemElements.length > 1) {
        ...
        this.itemIntersectionObserver = new IntersectionObserver(this._evtOnItemObserved.bind(this), { threshold: 0.4 });
    ...
    select(index, animate = true) {
      const listItemElement = this.listItemElements[index], apBoundingRect = listItemElement.getBoundingClientRect();
      if (animate) {
  ...
  var NavigationOfDesktop = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.apollo.on("mouseenter", ".has-dropdown", (event, target) => {
        if (event.target === target && event.relatedTarget !== null) {
          this.openDropdown(target);
        }
      }, true);
      this.apollo.on("shopify:block:select", (event) => this.openDropdown(event.target.parentElement));
      this.apollo.on("shopify:block:deselect", (event) => this.closeDropdown(event.target.parentElement));
    }
    openDropdown(parentElement) {
      const itemmenu = parentElement.querySelector("[ap-controlsaria]");
      const itemdropdown = parentElement.querySelector(`#${itemmenu.getAttribute("ap-controlsaria")}`);
      ...
      let timeoutOpen = setTimeout(() => {
        ...
        if (itemdropdown.classList.contains("mega-menu") && !MediaFeatures.prefersReducedMotion()) {
          const items = Array.from(itemdropdown.querySelectorAll(".mega-menu__column, .mega-menu__image-push"));
          items.forEach((item) => {
            item.getAnimations().forEach((animation2) => animation2.cancel());
            item.style.opacity = 0;
          });
          const animation = new ApAnimationCustom(new ApEffectParallel(items.map((item, index) => {
            return new ApEffectOfCustomKeyframe(item, {
              opacity: [0, 1],
    ...
    closeDropdown(parentElement) {
      ...
      requestAnimationFrame(() => {
        ...
        }, itemdropdown.classList.contains("mega-menu") && this.megaMenuCurrent !== itemdropdown ? 250 : 0);
  ...
  var ApolloMobileMenu = class extends ApDrawerContent {
    get apparitionAnimation() {
      ...
      if (!MediaFeatures.prefersReducedMotion()) {
        ...
        effects.push(new ApEffectParallel(navItems.map((item, index) => {
          return new ApEffectOfCustomKeyframe(item, {
            opacity: [0, 1],
        ...
        if (bottomBar) {
          effects.push(new ApEffectOfCustomKeyframe(bottomBar, {
            opacity: [0, 1],
            transform: ["translateY(100%)", "translateY(0)"]
          }, {
            duration: 300,
    ...
    attributeChangedCallback(name, oldval, newval) {
      ...
      switch (name) {
        case "open":
          ...
          ApolloEventTrigger(this, this.open ? "mobile-nav:open" : "mobile-nav:close");
      }
    }
  };
  ...
  var ApolloVerticalMenu = class extends ApDrawerContent {
    get apparitionAnimation() {
      ...
      if (!MediaFeatures.prefersReducedMotion()) {
        const navItems = Array.from(this.querySelectorAll('.vertical-nav__item[data-level="1"]')), effects = [];
        effects.push(new ApEffectParallel(navItems.map((item, index) => {
          return new ApEffectOfCustomKeyframe(item, {
            opacity: [0, 1],
            transform: ["translateX(-40px)", "translateX(0)"]
    ...
    attributeChangedCallback(name, oldval, newval) {
      ...
      switch (name) {
        case "open":
          if (this.open && this.apparitionAnimation) {
            Array.from(this.querySelectorAll('.vertical-nav__item[data-level="1"], .drawer__footer')).forEach((item) => item.style.opacity = 0);
            this.apparitionAnimation.play();
          }
          ApolloEventTrigger(this, this.open ? "vertical-nav:open" : "vertical-nav:close");
      }
    }
  };
  ...
  var ApSwipePhotoUi = class {
    constructor(pswp) {
      ...
      this.photoSwipeInstance.listen("initialZoomInEnd", this._evtOnInitialZoomInEndApSwp.bind(this));
      this.photoSwipeInstance.listen("initialZoomOut", this._evtOnInitialZoomOutApSwp.bind(this));
      this.photoSwipeInstance.listen("parseVerticalMargin", this._evtOnParseVerticalMarginApSwp.bind(this));
  ...
  var ApProductZoomImage = class extends ApolloOpenableElement {
    ...
    async _buildItemsSwipe() {
      const activeImages = Array.from(this.mediaElement.querySelectorAll('.product__media-item[data-media-type="image"]:not(.is-filtered)')), product = await ApLoaderProduct.load(this.getAttribute("product-handle"));
      return Promise.resolve(activeImages.map((item) => {
        ...
        const supportedSizes = apGetSupportedSizes(matchedMedia, [200, 300, 400, 500, 600, 700, 800, 1e3, 1200, 1400, 1600, 1800, 2e3, 2200, 2400, 2600, 2800, 3e3]);
        const desiredWidth = Math.min(supportedSizes[supportedSizes.length - 1], window.innerWidth);
        return {
          selected: item.classList.contains("is-selected"),
          src: apGetSizedOfMediaUrl(matchedMedia, `${Math.ceil(Math.min(desiredWidth * window.devicePixelRatio * this.maxSpreadZoom, 3e3))}x`),
  ...
  var ApContainerPayment = class extends HTMLElement {
    ...
    _evtUpdateAddToCartButton(variant) {
      ...
      addToCartObjectButtonElement.classList.remove("button--primary", "button--secondary", "button--ternary");
      if (!variant) {
        ...
        if (variant["available"]) {
          ...
          addToCartObjectButtonElement.classList.add(addToCartObjectButtonElement.hasAttribute("data-use-primary") ? "button--primary" : "button--secondary");
    ...
    _evtUpdateCheckoutButtonDynamic(variant) {
      ...
      paymentObjectButtonElement.style.display = !variant || !variant["available"] ? "none" : "block";
    }
  };
  ...
  var ApProductForm = class extends HTMLFormElement {
    ...
    async _onSubmitApProductForm(event) {
      ...
      this.dispatchEvent(new CustomEvent("ap-cartnotification:show", {
        bubbles: true,
        cancelable: true,
        detail: {
  ...
  var ApProductMedia = class extends CustomHTMLElement {
    async connectedCallback() {
      ...
      if (this.flickityCarousel.childElementCount === 1 || this.querySelector(".ap-product-media-list").getAttribute('list-type')) {
        return;
      }
      ...
      (_a = document.getElementById(this.getAttribute("form-id"))) == null ? void 0 : _a.addEventListener("variant:changed", this._evtOnChangedVariant.bind(this));
      this.flickityCarousel.addEventListener("model:played", () => this.flickityCarousel.setDraggable(false));
      this.flickityCarousel.addEventListener("model:paused", () => this.flickityCarousel.setDraggable(true));
      this.flickityCarousel.addEventListener("video:played", () => this.flickityCarousel.setDraggable(false));
      this.flickityCarousel.addEventListener("video:paused", () => this.flickityCarousel.setDraggable(true));
      // this.flickityCarousel.addEventListener("flickity:ready", this._evtOnFlickityReady.bind(this));
      this.flickityCarousel.addEventListener("flickity:slide-changed", this._eveOnFlickityChanged.bind(this));
      this.flickityCarousel.addEventListener("flickity:slide-settled", this._evtOnFlickitySettled.bind(this));
    ...
    async _setupTextOverlayImageVisibility() {
      ...
      const prefersReducedMotion = MediaFeatures.prefersReducedMotion();
      await imageLoaded(image);
      const animation = new ApAnimationCustom(new ApEffectParallel([
        new ApEffectOfCustomKeyframe(image, { opacity: [0, 1], transform: [`scale(${prefersReducedMotion ? 1 : 1.1})`, "scale(1)"] }, { duration: 500, easing: "cubic-bezier(0.65, 0, 0.35, 1)" }),
        new ApEffectParallel(Array.from(this.querySelectorAll(".product__thumbnail-item:not(.is-filtered)")).map((item, index) => {
          return new ApEffectOfCustomKeyframe(item, {
            opacity: [0, 1],
    ...
    async _evtOnChangedVariant(event) {
      ...
      this.product["media"].forEach((media) => {
        var _a;
        let matchMedia2 = productvariant["featured_media"] && media["id"] === productvariant["featured_media"]["id"];
        if ((_a = media["alt"]) == null ? void 0 : _a.includes("#")) {
          mediaReloadStatus = true;
          if (!matchMedia2) {
            const altofParts = media["alt"].split("#"), mediaGroupParts = altofParts.pop().split("_");
            this.product["options"].forEach((option) => {
              if (option["name"].toLowerCase() === mediaGroupParts[0].toLowerCase()) {
                if (productvariant["options"][option["position"] - 1].toLowerCase() !== mediaGroupParts[1].trim().toLowerCase()) {
      ...
      if (currentlyFilteredIds.some((value) => !mediaIdsFiltered.includes(value))) {
        const selectedMediaId = productvariant["featured_media"] ? productvariant["featured_media"]["id"] : this.product["media"].map((item) => item.id).filter((item) => !mediaIdsFiltered.includes(item))[0];
        Array.from(this.querySelectorAll("[data-media-id]")).forEach((item) => {
          item.classList.toggle("is-filtered", mediaIdsFiltered.includes(parseInt(item.getAttribute("data-media-id"))));
        ...
        this.flickityCarousel.select(`[data-media-id="${event.detail.variant["featured_media"]["id"]}"]`);
      }
      ...
      if (!variant) {
        ...
        if (!variant["available"]) {
          ...
          if (window.themeVariables.settings.discountMode === "percentage") {
            priceDiscount = `${Math.round((variant["compare_at_price"] - variant["price"]) * 100 / variant["compare_at_price"])}%`;
          } else {
            priceDiscount = apMoneyFormat(variant["compare_at_price"] - variant["price"]);
          }
          productListLabel.innerHTML = `<span class="label label--highlight">${window.themeVariables.strings.collectionDiscount.replace("@savings@", priceDiscount)}</span>`;
        }
      }
    }
    _evtPricesUpdate(variant) {
      ...
      if (!variant) {
        ...
        if (variant["compare_at_price"] > variant["price"]) {
          listProductPrices.innerHTML += `<span class="price price--highlight ${this.priceClass}"><span class="visually-hidden">${window.themeVariables.strings.productSalePrice}</span>${apMoneyFormat(variant["price"], currencyFormat)}</span>`;
          listProductPrices.innerHTML += `<span class="price price--compare"><span class="visually-hidden">${window.themeVariables.strings.productRegularPrice}</span>${apMoneyFormat(variant["compare_at_price"], currencyFormat)}</span>`;
        } else {
          listProductPrices.innerHTML += `<span class="price ${this.priceClass}"><span class="visually-hidden">${window.themeVariables.strings.productSalePrice}</span>${apMoneyFormat(variant["price"], currencyFormat)}</span>`;
        }
  ...
  var ApProductVariants = class extends CustomHTMLElement {
    ...
    selectVariant(id) {
      ...
      this._updateDisableSelectors();
      ApolloEventTrigger(this.masterSelector.form, "variant:changed", { variant: this.selectedVariant });
    }
    _evtOnChangedOption() {
      var _a;
  ...
  var ProductItem = class extends CustomHTMLElement {
    connectedCallback() {
      ...
      this.apollo.on("change", ".product-item-meta__swatch-list .color-swatch__radio", this._evtOnChangedColorSwatch.bind(this));
      this.apollo.on("mouseenter", ".product-item-meta__swatch-list .color-swatch__item", this._evtOnHoveredColorSwatch.bind(this), true);
    }
  ...
  var ApProductFacet = class extends CustomHTMLElement {
    ...
    async _rerenderApProductFacet(event) {
      ...
      try {
        ...
        const activeFilterList = Array.from(fakeDiv.querySelectorAll(".ap-productfacet__active-list")), toolbarItem = document.querySelector(".mobile-toolbar__item--filters");
        if (toolbarItem) {
        ...
        if (filtersTempDiv) {
          const previousScrollTop = this.querySelector("#ap-facetfilters .drawer__content").scrollTop;
          Array.from(this.querySelectorAll("#ap-facetfilters-form .collapsible-toggle")).forEach((filterToggle) => {
            const filtersTempDivToggle = filtersTempDiv.querySelector(`[ap-controlsaria="${filterToggle.getAttribute("ap-controlsaria")}"]`), expandedStatus = filterToggle.getAttribute("ap-expanded-aria") === "true";
            if (expandedStatus) {
              let ex_s = "true";
            } else {
              let ex_s = "false";
        ...
        const scrollTo = this.querySelector(".ap-productfacet__meta-bar") || this.querySelector(".ap-productfacet__ap-productlist") || this.querySelector(".ap-productfacet__main");
        requestAnimationFrame(() => {
  ...
  var ApFacetFilters = class extends ApDrawerContent {
    connectedCallback() {
      ...
      this.rootApollo.on("click", '[data-action="clear-filters"]', this._evtOnClearedFilters.bind(this));
    ...
    _evtOnClearedFilters(event, target) {
      event.preventDefault();
      ApolloEventTrigger(this, "facet:criteria-changed", { url: target.href });
    }
    _evtOnChangedFilter() {
      ...
      ApolloEventTrigger(this, "facet:criteria-changed", { url: `${window.location.pathname}?${searchParamsAsString}` });
    }
  ...
  var ApCartCount = class extends CustomHTMLElement {
    connectedCallback() {
      this.rootApollo.on("cart:updated", (event) => this.innerText = event.detail.cart["item_count"]);
    }
    ...
    async _detailCart(event) {
      ...
      console.log(event.detail.cart['total_price']);
    ...
    async _rerenderCart(event) {
      ...
      setTimeout(async () => {
        ...
        if (cartContent && cartContent["item_count"] === 0) {
          const animation = new ApAnimationCustom(new ApEffectParallel(Array.from(this.querySelectorAll(".drawer__content, .drawer__footer")).map((item) => {
            return new ApEffectOfCustomKeyframe(item, { opacity: [1, 0] }, { duration: 250, easing: "ease-in" });
          })));
          animation.play();
          await animation.finished;
        }
        ...
        case "open":
          if (this.open) {
            ...
            if (!MediaFeatures.prefersReducedMotion()) {
              ...
              effects.push(new ApEffectParallel(lineItems.map((item, index) => {
                return new ApEffectOfCustomKeyframe(item, {
                  opacity: [0, 1],
                  transform: ["translateX(40px)", "translateX(0)"]
                }, {
              ...
              if (recommendationsInner && !window.matchMedia(window.themeVariables.breakpoints.pocket).matches) {
                effects.push(new ApEffectOfCustomKeyframe(recommendationsInner, {
                  opacity: [0, 1],
  ...
  var ApCartNote = class extends HTMLTextAreaElement {
    ...
    get ownedToggle() {
      return this.hasAttribute("ap-ariaowns") ? document.getElementById(this.getAttribute("ap-ariaowns")) : null;
    }
  ...
  var ApCartNotification = class extends CustomHTMLElement {
    connectedCallback() {
      this.rootApollo.on("ap-cartnotification:show", this._evtOnShow.bind(this), !this.hasAttribute("global"));
      this.apollo.on("click", '[data-action="close"]', (event) => {
        event.stopPropagation();
        this.hidden = true;
      });
  ...
  var ApShippingEstimator = class extends HTMLElement {
    ...
    _formatRatesShipping(shippingRates) {
      ...
      shippingRates.forEach((shippingRate) => {
        formattedShippingRates += `<li>${shippingRate["presentment_name"]}: ${apMoneyFormat(parseFloat(shippingRate["price"]) * 100)}</li>`;
      });
      const html = `
  ...
  var ApReviewLink = class extends HTMLAnchorElement {
    ...
    _onClick() {
      ...
      if (window.matchMedia(window.themeVariables.breakpoints.pocket).matches) {
        ...
        document.querySelector(`[ap-controlsaria="${shopifyReviewsElement.closest(".product-tabs__tab-item-wrapper").id}"]`).click();
      }
    }
  };
  ...
  var ApProductStickyForm = class extends HTMLElement {
    ...
    set hidden(value) {
      ...
      if (value) {
        ...
        document.documentElement.style.setProperty("--ap-cartnotification-offset", `${this.clientHeight}px`);
      }
    }
    _evtOnChangedVariant(event) {
      const variant = event.detail.variant;
    ...
    _setupVisibilityObserversTextOverlayImage() {
      const paymentContainerElement = document.getElementById("ApolloProductSticky"), footerElement = document.querySelector(".shopify-section--footer"), apOffsetStickyHeader = apStickyHeaderOffsetContent();
      ...
      this.intersectionObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.target === paymentContainerElement) {
            ...
            this._isApContainerPaymentPassed = entry.intersectionRatio === 0 && apBoundingRect.top + apBoundingRect.height <= apOffsetStickyHeader;
          }
        });
        if (window.matchMedia(window.themeVariables.breakpoints.pocket).matches) {
  ...
  (() => {
    ...
    let setPropertyViewport = () => {
      ...
      const clientHeight = window.visualViewport ? window.visualViewport.height : document.documentElement.clientHeight;
  ...
  (() => {
    ...
    documentApollo.on("keyup", 'input:not([type="checkbox"]):not([type="radio"]), textarea', (event, target) => {
  ...
  if (document.getElementById('instafeed')) {
    ...
    if (token) {
      ...
      let feed = new Instafeed({
        ...
        limit: limit,
        template: '<div class="ap-multicolumn__item ap-multicolumn__item--align-start " reveal><div class="ap-multicolumn__image-wrapper"><img class="ap-multicolumn__image" alt="{{caption}}" src="{{image}}" ></div></div>',
